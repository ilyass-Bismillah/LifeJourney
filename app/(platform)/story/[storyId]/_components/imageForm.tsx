...
const ImageForm = ({ initialData }: Props) => {
  ...
  const onSubmit = async (image: { image: string }) => {
    try {
      ...
      if (res.status === 200) {
        toast("Story updated", { className: "bg-emerald-500 text-white" });
        toggleEdit();
        router.refresh();
      }
    } catch (error) {
      console.log(error);
      toast("Somtrhing went wrong", { className: "bg-rose-500 text-white" });
    }
  };
  return (
    <div className="border border-secondary rounded-md p-4 bg-secondary w-80">
      ...
      {!isEditing &&
        (!initialData.image ? (
          <div className="h-60 border-2 border-gray-300 rounde-md flex items-center justify-center">
